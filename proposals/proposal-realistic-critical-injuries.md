...
| III | `51-62` | Severe crippling | Lost limb, pelvic/spinal damage, catastrophic brain trauma, or dependence on constant care. |
| IV | `63-66` | Attribute-collapse survival | The character lives, but one or more governing attributes are permanently reduced to `1`. |
...
| 35-36 | Loss of hearing in one ear | `-2` to `SCOUTING` when locating sound; ambush defense rolls are `-1`. |
...
| 61-62 | Cascading organ failure | `-2` to `ENDURANCE`, `MOVE`, and recovery; without regular care, lose `1` Strength every week. |
| 63-64 | Quadriplegic scar survivor | Alive and lucid, but your `Strength` and `Agility` are both permanently reduced to `1`. |
| 65-66 | Systemic collapse survivor | The body barely holds together; your `Strength` and `Agility` are both permanently reduced to `1`, and each day of travel inflicts `1` Strength damage unless you ride or are carried. |
...
| 33-34 | Chronic internal bleeding | At the end of any combat where you took Strength damage, roll `ENDURANCE` or suffer `1` extra Strength damage. |
...
| 63-64 | Penetrating brain trauma | Your `Strength` and `Agility` are both permanently reduced to `1`; memory and speech problems may remain as roleplay fallout. |
| 65-66 | Heart kept beating, life not restored | The body survives on the edge of failure; your `Strength` and `Agility` are both permanently reduced to `1`. |
...
| 31-32 | Chronic concussion | `-1` to all `Wits` rolls and `SCOUTING`; each horror critical adds `+10`. |
| 33-34 | Crushed hand | Cannot use shields or two-handed weapons on that side; delicate work is impossible. |
| 35-36 | Crushed foot | `RUN` becomes slow and travel pace is reduced by one step. |
| 41-42 | Destroyed hip | Without a crutch or mount you cannot keep up with a marching party. |
| 43-44 | Damaged spine | `MIGHT`, `MOVE`, and `ENDURANCE` are each `-1`; heavy armor counts as one step heavier for you. |
| 45-46 | Traumatic brain injury | `Wits` maximum is reduced by `1`; memory, speech, or concentration deficits should be chosen with the player. |
...
| 55-56 | Partial paralysis | Below-the-waist movement requires assistance, cart transport, or a dedicated mobility device. |
| 61-62 | Severe brain damage | The character survives, but reliable judgment, speech, and independence are gone. |
| 63-64 | High spinal paralysis | Your `Agility` is permanently reduced to `1`. |
| 65-66 | Crushed torso survivor | Your `Strength` and `Agility` are both permanently reduced to `1`. |
...
| 13-14 | Destroyed sweat response | Forced march, heat, and heavy armor each become one category harsher for you. |
...
| 61-62 | Multi-limb contracture | You remain alive but cannot travel or fight without constant assistance. |
| 63-64 | Burned lungs and brain | Your `Strength` and `Agility` are both permanently reduced to `1`. |
| 65-66 | Survivable ruin | Your `Strength` and `Agility` are both permanently reduced to `1`, and all weather counts as one category harsher for you. |

## Additional Damage Families

The core book currently folds several non-typical harms together. If you want stronger realism, split out the following families instead of letting one burn table carry all of them.

Recommended gateway logic:

| Table | `65` result | `66` result |
| --- | --- | --- |
| Horror | Permanent trauma. Roll on Permanent Trauma - Horror. | The mind or heart gives way entirely. Death or permanent `Wits 1` and `Empathy 1`. |
| Poison | Catastrophic toxic injury. Roll on Permanent Injuries - Poison. | Total organ collapse or irreversible brain injury. Death or permanent invalidism. |
| Acid / Corrosion | Catastrophic corrosive injury. Roll on Permanent Injuries - Acid / Corrosion. | Destruction beyond survival. |
| Cold / Freeze | Catastrophic freezing injury. Roll on Permanent Injuries - Cold / Freeze. | Whole-body freezing trauma, unsurvivable tissue death, or terminal collapse. |
| Swallow | Catastrophic maw-and-gut trauma. Roll on Permanent Injuries - Swallow. | The victim is crushed, drowned, digested, or suffocated beyond rescue. |
...
| 53-54 | Dissociative spell | No | - | The first time you suffer damage or a fear attack in an encounter, roll `INSIGHT` or lose your next slow action. | 2D6 |
...
| 65 | Permanent trauma onset | No | - | Roll on Permanent Trauma - Horror. | Permanent |
...
| 51-52 | Toxic lung injury | Yes | D6 days | `-2` to `ENDURANCE` and `MOVE`. | D6 |
...
| 65 | Catastrophic toxic injury | Yes | D6 hours | Roll on Permanent Injuries - Poison. | Permanent |
...
| 65 | Catastrophic corrosive injury | Yes | D6 rounds | Roll on Permanent Injuries - Acid / Corrosion. | Permanent |
...
| 53-54 | Deadened nerves | Yes | D6 days | Each `Strength` ⚔️ inflicts damage, even if the roll is not pushed. | D6 |
...
| 65 | Catastrophic freezing injury | Yes | D6 hours | Roll on Permanent Injuries - Cold / Freeze. | Permanent |
...
| 65 | Catastrophic maw-and-gut trauma | Yes | D6 rounds | Roll on Permanent Injuries - Swallow. | Permanent |
...
| 21-26 | Hypervigilance | You are never off watch. You gain `+1` to `SCOUTING` against ambush, but you cannot recover `Wits` from sleep unless another person relieves you and you are in a secure shelter. |
...
| 55-56 | Dissociation under stress | The first time in an encounter that you suffer damage or a fear attack, roll `INSIGHT` or lose your next slow action as the world goes distant and unreal. |
...
| 65 | Sleep cannot hold you | You cannot benefit from ordinary sleep without herbs, drink, narcotics, magic, or active reassurance. Without help, every dawn costs you `1` Wits. |
| 66 | Fractured self | Your `Wits` and `Empathy` are both permanently reduced to `1`. You can still speak and make choices, but any further mental damage Breaks you immediately. |
...
| 55-56 | Seizure disorder | The first time each day that an attribute reaches `0`, roll `ENDURANCE`. On a failure, you collapse and lose your next round. |
...
| 63-64 | Partial paralysis | The toxin leaves lasting nerve death. `MOVE` and one of `MELEE` or `MARKSMANSHIP` are both `-2`, and travel without support is slow. |
| 65 | Organ failure survivor | Your `Strength` is permanently reduced to `1`. |
| 66 | Toxic brain injury | Your `Strength` and `Agility` are both permanently reduced to `1`, and tremor or confusion may remain as roleplay fallout. |
...
| D66 | Permanent Injury | Lasting Effect |
| --- | --- | --- |
| 11-16 | Mouth and throat scarring | `MANIPULATION` is `-2`, and shouting warnings or power words is impossible above a rasp. |
| 21-26 | Esophageal stricture | Eating takes twice as long, hard rations are painful, and every forced march requires `ENDURANCE` or inflicts `1` Strength damage from retching and exhaustion. |
...
| 55-56 | Circulation failure | Each time you become `COLD`, you immediately take `1` extra Strength damage and recover that Strength only after full shelter and rest. |
...
| 65 | Recurrent freezing injury | Your `Agility` is permanently reduced to `1`, and each time you become `COLD` you also take `1` Strength damage. |
| 66 | Multi-site freezing ruin | Your `Strength` and `Agility` are both permanently reduced to `1`. |
...
| D66 | Permanent Injury | Lasting Effect |
| --- | --- | --- |
| 11-16 | Aspiration-scarred lungs | `ENDURANCE` is `-2`, and drowning or choking effects against you worsen by one step. |
...
| 61-62 | Oxygen-starved brain injury | `Wits` maximum is reduced by `1`, and concentration-heavy actions are at `-1`. |
| 63-64 | Pelvic or spinal crush | Your `Agility` is permanently reduced to `1`. |
| 65 | Digestive fistula survivor | Your `Strength` is permanently reduced to `1`. |
| 66 | Broken by the gut | Your `Strength` and `Agility` are both permanently reduced to `1`. |
