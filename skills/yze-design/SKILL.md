...
name: yze-design
description: |
  Use when designing, abstracting, extrapolating, or INVENTING mechanics for the Year Zero Engine (YZE) by Free League Publishing — the D6 dice-pool RPG engine shared by Forbidden Lands 2E and Tales of the Old West 2E. Reverse-engineers both corebooks into one engine-agnostic design studio in four layers: UNDERSTAND — core resolution, character, conflict, harm, power, travel, orgs, gear, GM procedures, philosophy, dials, the FL-vs-West divergence map; ORCHESTRATE — route work into analysis, host-game adaptation, complete-game build, subsystem invention, or validation; INVENT — reusable mechanical primitives, advanced compositions, a dual-use matrix, and a 5-operator reinvention method for transplanting primitives into new systems; VALIDATE/PUBLISH — executable math, exploit, attrition, table-feel, and publication-handoff procedures. Use it to build complete YZE games, extend existing YZE-style games, invent modular frameworks, validate balance/feel, and write publication-ready rules.
---
...
- `references/22-design-orchestrator.md` — **Load first for any substantial task.** Routes the request into one of five modes: analyze existing mechanic, adapt existing YZE game, build complete new game, invent subsystem, validate/audit subsystem. Defines mandatory outputs, stop-and-ask gates, five-loop proof, host-game extension contract, and publication handoff.
...
- `references/24-validation-worksheets.md` — **Load when checking math, attrition, resource loops, action economy, exploits, table burden, or felt experience.** Reusable worksheets, threshold warnings, and simulation-lite formulas.
...
- `references/07-base-faction-mass-scale.md` — **Load when** designing the downtime org layer. Strongholds, Towns/Property, Outlaw Bands, Mercenary Companies, Factions+Legacies, Mass Combat — abstracted as founding/functions/upkeep/events/scale-escalation, with operating sheets and resolution loops for outlaw bands, mercenary companies, and host-scale play.
...
- `references/08-gear-and-economy.md` — **Load when** designing gear, inventory, transport, crafting, construction, property, investment, trade, gambling, or economy. Supply/scarcity tables, economy models (barter vs cash+capital vs gold/silver), weapon/armor feature grammar, quality tiers, artifact/legendary dice, consumables-as-resource-dice, carry limits, containers, pack animals, wagons/boats, group stores, stockpiles, storage functions, crafting gates, stronghold functions, property Status, town amenities, Capital, loans, salaries, cargo runs, market rolls, gambling scenes, upkeep, payback, and economic validation.
...
- `references/14-recipes-new-game-new-rule.md` — **Load when** starting a new game or rule from scratch. Two cookbooks: (1) Build a complete YZE game package for any genre. (2) Design + validate a new rule/subsystem (intent→math→exploit→integration→readability).
...
- `references/16-mechanical-primitives.md` — **Load first when inventing a new mechanic.** The palette: 15 proven-portable core patterns plus advanced compositions (clocks/fronts, relationship edges, revelation ladders, stance tracks, heat/wanted pressure, vow/oath engines, crew-role menus, vehicle-as-character, territory control, rumor economies, reputation propagation, ritual/project tracks). Each entry names its shape test, parameters, math/tempo notes, and transplants.
...
### Workshop — drop-in subsystem library

> The `references/` folder documents the engine. The `workshop/` folder **extends** it: a growing library of ready-to-use, engine-agnostic subsystems built by applying the Reinvention Method (`18`) to the primitives (`16`). Each module is genre-agnostic in its **core** (the bulk of the file) with one **worked genre example** showing it calibrated and skinned. Read `workshop/README.md` for the index and usage method; `workshop/00-module-template.md` for the structure every module follows. Drop a module into any YZE-family game by setting its dials and swapping nouns (per `15`).
...
- `workshop/03-social-combat-as-real-combat.md` — High-stakes social scenes as full tactical conflicts: the action economy, social-distance "range bands," leverage as weapons, Composure as social HP, a Broken-equivalent (the combat engine domain-transferred). *Example: Regency high society.* Use for: diplomacy, courtroom, interrogation, high-society genres.
...
- `workshop/05-investigation-and-clue-economy.md` — Clues as a capped metacurrency (P2) earned via an inquiry activity menu (P6) and spent on a revelation ladder; risky inferences use the push (P1). *Example: noir / cosmic horror.* Use for: mysteries, procedurals, research-driven campaigns.
- `workshop/06-debt-and-obligation.md` — An *inverted* metacurrency (P2 inverted): debts grow when you take favors and must be paid down; called debts are the GM's pressure valve. Unifies financial/social/moral/supernatural debt. *Example: corporate space opera.* Use for: any game where obligations are the engine of plot.
...
- `workshop/17-monsters-threats-and-boss-architecture.md` — Threat design from primitives rather than stat inflation: clocks, phases, tells, lairs, and consequence payloads. Use for: horror, boss fights, kaiju, nemeses.
