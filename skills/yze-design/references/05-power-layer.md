...
> **STATUS: FILLED (Pass-1 extraction + Pass-2 abstraction complete).** The power layer is the engine's largest *optional* subsystem. This file treats it as a plug-in that sits on top of `00-engine-core.md` — and proves, via West, that the engine runs at full strength with the plug-in removed.

## Contents

1. Source provenance
2. Abstraction target
3. The power taxonomy (Spells / Power Words / Rituals / Miracles)
4. Casting models (safe / chance / overcharge)
5. The metacurrency as power fuel (WP + Burn)
...
- `01-corebook/03-skills.md:120-138` — the Willpower (WP) metacurrency that *fuels* magic, its cap (10), and its refuel trigger (1 WP per 💀 on Base Dice when pushing).

**Tales of the Old West 2E (West):**
- `01-corebook/03-rolling-the-bones.md:258-311` — **no magic system exists.** West is the engine's **zero instance**: proof the core loop, the metacurrency, the push, the Trouble layer, and the conflict model all run without any power layer. The nearest analogue is **Faith** (a metacurrency, not a power source) and a non-mechanical religious frame. This absence is itself the deliverable's anchor point.
...
1. **The on/off + density dial** (§10) — *the single most important output.* A decision table mapping genre → power-layer density (none / low / medium / high) → taxonomy subset → casting model → fuel model. West = none; FL = high; the middle is mapped for low-magic, psi, gadgets, and miracles.
2. **The power taxonomy** (§3) — a reusable three-tier effect-speed frame: Spells (slow) / Power Words (fast-reactive) / Rituals (long-dramatic), plus a possible Miracles tier.
3. **The casting-model dial** (§4) — safe (guaranteed-but-costly) vs chance (rolled-but-cheap) vs overcharge (pushed-for-scale).
4. **The fuel + cost model** (§5) — metacurrency + the Burn self-harm override.
...
Apply the Abstraction Ladder (source instance → generic mechanism → design intent → dials with FL/West as two points) to each.

## 3. The power taxonomy (Spells / Power Words / Rituals / Miracles)

FL sorts every power into one of **three types** by *effect speed* and *action economy*, not by school or element. This sorting is the reusable frame — it is about *when the effect resolves in the turn structure*, which is the axis the core engine already cares about (fast vs slow action, out-of-turn reactions).

| Type | FL action economy | Role in the turn | FL source |
| --- | --- | --- | --- |
...
**Generic mechanism — a three-tier effect-speed taxonomy:**
...
- **Tier 2 — Reactive powers** (FL "Power Words"): fast, and crucially *out-of-turn / interrupting*. This tier exists so a power-user can defend like a fighter — it is the engine's defensive-reaction slot (parry/dodge) opened to magic. *This is the tier most often omitted in lighter power systems*, and its presence/absence is a real dial: without it, casters are glass cannons who must pre-cast or die.
...
**Generic mechanism — the casting-risk dial (three positions):**
...
**FL-vs-West divergence:** West has no casting roll at all because it has no powers. But note the *structural kinship*: West's Faith-spend-to-push and FL's WP-spend-to-cast are the *same economy* (capped metacurrency → bigger/more-reliable effect). The power layer simply adds a third thing the currency can buy (an effect that didn't exist before), plus a risk face (💀) tuned to magical backlash rather than bodily harm. **Layer:** the casting-risk dial is **General** when the layer is on; you must pick at least one position (FL uses all three).

## 5. The metacurrency as power fuel (WP + Burn)

The power layer does **not** introduce a new currency. It spends the *same* metacurrency the core engine already mints (see `00-engine-core.md` §7). FL's Willpower Points — capped at 10, refueled 1-per-💀-on-Base-Dice-when-pushing — are simultaneously the fuel for kin/profession talents *and* the fuel for every spell. FL `03-skills.md:120-126`; `07-magic.md:73-75`.

**This is the key coupling:** the power layer is *downstream* of the core risk loop. You cast spells with WP; you *earn* WP by taking damage on pushed rolls. A caster who never pushes never has fuel. Magic is, mechanically, *the thing you spend your suffering on.* FL `03-skills.md:124`.
...
**The decisive design twist:** "You choose how much to burn, but you do not choose where the damage falls." FL `07-magic.md:131`. The *quantity* is player-controlled (a clean cost-benefit dial); the *location* is random (so burning is never a solved optimization — it might cripple the attribute you needed to survive the aftermath).

**Generic mechanism — fuel + the self-harm override:**
- **Fuel:** a power costs units of the core metacurrency (the same pool that pays for pushing and talents). This *binds the power layer to the core loop* — you cannot spam powers without engaging the engine's risk mechanics, because that is where fuel comes from.
- **Self-harm override (Burn):** an optional escape valve that lets a caster cast *without* currency by paying in attribute damage instead. Two sub-dials: (a) **player-chosen quantity vs random target** — FL picks the elegant middle (chosen amount, random attribute), which preserves tension; (b) **temporary vs permanent** damage. FL uses normal (recoverable) attribute damage; epic ingredients (§9) escalate to *permanent* attribute loss.
- **Refund mechanics (optional):** FL's *Absorb Magical Residue* (rank 5, `07-magic.md:456-463`) and *Transfer* (rank 3, `:380`) let casters reclaim or move WP — the layer's internal recycling, gated behind high rank so it cannot trivialize the fuel economy at low levels.

**FL-vs-West divergence:** West's Faith fuels *pushing and Trouble-buyoff* — it never buys an effect that a mundane character couldn't attempt. FL's WP buys *entirely new capabilities* (flight, resurrection, demon-summoning). The metacurrency is identical in shape (capped 10, spent for advantage); what differs is the *scope of what it can purchase*. This is the cleanest single proof that the power layer is an additive extension of the same economy, not a parallel system. **Layer:** fuel-from-core-metacurrency is **General** (when the layer is on); Burn is **Optional** (a grit/ desperation dial — omit for lighter power systems that just gate by WP).
...
**The mishap engine (shared mechanics across all families):**
...
**Generic mechanism — the failure-pressure layer for powers (cross-link to `04-harm-and-consequences.md`):** this is the *same pattern* as the typed-critical-injury family. The recipe:
...
3. Apply a **count-based modifier** to the roll (FL: −10/+0/+10 by 💀 count) so that "more things going wrong" *mechanically* pushes toward the worse rows, not just "more rolls."
...
**Forging new spells (the design appendix):** FL ships a complete **spell-design subsystem** (FL `07-magic.md:5254-5452`). A caster at least one rank above the target spell collaborates with the GM over 5 days × rank (5 successful WITS rolls per rank). It includes:
...
**The on/off switch is binary; the density is a four-point dial.** A game either has powers (on) or does not (off). If on, density selects how much of the FL apparatus to instantiate.

### The density decision table

| Density | Power layer present? | Taxonomy tiers | Casting model | Fuel model | Mishap layer | Rank ladder | Epic tier | Genre fit | Exemplar |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **None** | **No.** The engine runs on core loop + metacurrency + Trouble alone. | — | — | metacurrency fuels *push/talents only* | none (use core Trouble) | — | — | Historical, mundane survival, heist, western, war, sports | **West** |
| **Low** | Yes, thin. | Standard (Spells) only — often reskinned as gadgets / psi / knacks | Safe-only (guaranteed, no casting roll) or a single flat roll | metacurrency; no Burn | Single generic mishap table (or core Trouble reskinned) | 1–3 | No | Low-magic fantasy, pulp gadgets, mild psi, "lucky" abilities | gadget/psi reskin |
| **Medium** | Yes, moderate. | Standard + Rituals (drop Power Words, or keep them rare) | Safe + Overcharge | metacurrency + Burn (optional) | 2–4 typed mishap families (per source) | 1–5 | Optional (rank 5 cap) | Sword & sorcery, urban fantasy, superheroic, miracles+clerics | cut-down FL |
| **High** | Yes, maximal. | All three tiers (Spells + Power Words + Rituals) | Safe + Chance + Overcharge | metacurrency + Burn + refunds | Full typed families (one per discipline/source) | 1–6 | Yes (epic ingredients) | High fantasy, mythic, kitchen-sink magic | **FL** |

### How to read the table

- **None (West):** the zero instance. West's Faith (`03-rolling-the-bones.md:258-311`) is a *metacurrency that fuels pushing and Trouble-buyoff* — it never buys an effect a mundane character couldn't attempt. The engine's push-cost model, Trouble layer, opposed rolls, and conflict system are *completely self-sufficient.* This is not a deficiency; it is the proof that the power layer is **purely additive.** Choose None for any genre where "special abilities" would break the fiction (historical, western, war, survival, mystery, sports, social-drama).
...
2. **If yes — is the power *specialist* or *universal*?** If only some characters have powers (wizards in a party of fighters), you can afford **Medium/High** density because the power layer is one character's subsystem. If *everyone* has a power (psi/gadgets for all), lean **Low** so the layer doesn't dominate every turn.
3. **Pick the taxonomy tiers.** Always Standard. Add Rituals if the genre has ceremonial/downtime magic. Add Power Words only if power-users need to defend like fighters (otherwise they are glass cannons).
4. **Pick the casting model(s).** Safe-only for Low; Safe+Overcharge for Medium; all three for High.
5. **Pick the fuel + cost.** Always the core metacurrency (this is what binds the layer to the engine). Add Burn for Medium+ if you want desperate casting. Add refunds only at High.
...
| --- | --- | --- | --- | --- |
| **Power layer on/off** | **ON** — full apparatus | **OFF** — zero instance | Engine + maximal subsystem vs engine alone | Off for any genre where powers break the fiction; on when powers *are* the fiction |
| **What the metacurrency buys** | Pushing, talents, **and entire new capabilities** (spells) | Pushing and Trouble-buyoff **only** | Extended purchase scope vs core-scope only | Extend scope only if you turn the layer on |
...
| **Power taxonomy** | Three tiers (Spells / Power Words / Rituals) | None | Tactical + reactive + ceremonial | Include all three for High; prune for Low/Medium |
| **Casting-risk dial** | Safe + Chance + Overcharge | None | Guaranteed-costly / rolled-cheap / pushed-scale | Pick ≥1 when the layer is on |
| **Fuel** | Core WP (capped 10, harm-refueled) | Core Faith (capped 10, action/success-refueled) | Same economy, different refuel loop | Reuse the core metacurrency — do not invent a new one |
...
2. **Density** — none / low / medium / high. *(Sets every dial below by default; override per-genre.)*
3. **Taxonomy tiers** — {Standard} / {Standard, Rituals} / {Standard, Power Words, Rituals}. *(Effect-speed coverage.)*
4. **Casting-risk model** — safe / chance / overcharge — pick a subset. *(Variance and backlash.)*
5. **Fuel** — core metacurrency (always, when on); add Burn / refunds as desperation and recycling dials. *(Binds the layer to the engine's risk loop.)*
...
12. **Source axis** — single-source / split (arcane vs divine vs primal vs psionic). *(Optional orthogonal taxonomy — usually unnecessary.)*

**Instantiation recipe (any genre):**
1. **Decide on/off (dial 1).** If the genre's fiction is broken by special abilities, turn it off — you have West, and the engine is complete.
2. **If on, set density (dial 2)** using the §10 decision procedure (specialist vs universal power; genre's relationship to variance and backlash).
3. **Wire fuel to the core metacurrency (dial 5)** — never invent a parallel currency; the layer's elegance is that it spends the *same* pool the core loop mints.
4. **Pick the casting-risk model(s) (dial 4)** consistent with density and tone (safe for grounded; overcharge for variable; chance for "reach beyond your skill").
5. **Pick the taxonomy tiers (dial 3)** — decide explicitly whether power-users get a reactive tier (Power Words); this single choice determines whether casters can defend themselves.
6. **Build the mishap layer (dial 6)** to match the failure-pressure tone — typed families for "specific and memorable," core Trouble for "light and narrative."
7. **Add gating (dials 7–8)** only for the scarcity types your genre cares about (gadgets → ingredients; scholarly magic → grimoires; forbidden magic → secrecy).
...
The FL-vs-West divergence is the engine's proof that **the same spine supports both a game with no magic at all and a game with seventeen magical disciplines** — and that the difference is a set of *dials*, not a different engine. A new genre's job is to set the on/off switch and the density dial, wire the fuel to the core metacurrency, and then decide how much failure pressure and how high a ceiling its fiction can bear.
