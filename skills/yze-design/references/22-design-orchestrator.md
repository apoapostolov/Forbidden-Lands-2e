...
6. Subsystem invention contract
...
| --- | --- | --- | --- |
| **Analyze existing mechanic** | "What is this rule doing?", "abstract this", "why does this work?" | `00`-`15`, then `12` and `15` | Abstraction ladder + dial map + source-backed verdict |
| **Adapt existing YZE game** | "Add X to this game", "make a module for Alien/Vaesen/Coriolis-like play" | `23`, host game's own skill/docs if present, `13`, `20`, `21` | Host extension contract + rule text + validation memo |
| **Build complete new game** | "Make a YZE game about X" | `23`, `14`, `16`-`18`, selected workshop modules, `24` | Complete game package |
| **Invent subsystem** | "Create a mechanic for X" | `16`, `17`, `18`, relevant workshop modules, `24` | Modular subsystem with interface + rule text |
| **Validate/audit subsystem** | "Is this balanced?", "stress test this" | `13`, `19`, `24` | Verdict, failure points, fixes, revised rule if requested |

If a request looks like multiple modes, route to the highest-level one: complete game > host adaptation > subsystem invention > validation > analysis.
...
- **Engine profile:** FL anchor, West anchor, or one profile from `23`.
- **Five-loop map:** Expedition, Conflict, Recovery, Metacurrency, Base each marked Active / Secondary / Collapsed, with one-line reason.
- **Primitive map:** P1-P15 and advanced compositions used.
- **Subsystem inventory:** General, Situational, Optional, Campaign Mode.
- **Validation memo:** math/tempo, exploit surface, synergy, table burden, felt experience.
...
- 16-skill grid.
- Push cost, metacurrency, protected dial, and failure pressure.
...
| Metacurrency | What risk refuels agency, and what agency does it buy? |
...
- Three examples of play: ordinary roll, pushed roll, signature subsystem.
...
Run `24` worksheets and `13`/`19` protocols. Then load `20` and `21` to translate core mechanics into publication prose.
...
2. Pick one primary primitive and at most two supporting primitives from `16`.
3. Generate 3 candidate mechanics using `18`; for high-creativity requests generate 5 or 10.
4. Compare each candidate against a lighter reskin.
5. Select the smallest candidate that changes play behavior.
6. Write module interface: inputs required, outputs created, systems touched, incompatibilities.
7. Validate with `24`.
8. Publish using `20`/`21`.
...
1. **Player rule:** what the player does, in rulebook voice.
2. **GM procedure:** when to call for it, how to adjudicate edge cases.
3. **Designer note:** why it exists, where it hooks, validation caveats.
4. **Example of play:** one short example showing the decision and cost.
5. **Quick reference:** action names, costs, triggers, recovery, sheet fields.

Design jargon belongs in designer notes, not player rules. Terms such as "pressure loop," "primitive," "dial," and "validation pipeline" must be translated before publication.
