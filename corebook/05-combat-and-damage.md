...
At the start of each round, every participant rolls **AGILITY** and **WITS** together. Add the number of ⚔️ to your unmodified **AGILITY** (ignore any current damage). The total is your initiative segment for that round. Higher segments act before lower segments, and you can always delay to a lower segment. If you delay, you can drop to any lower segment at any time, but once you act you stay in that segment for the rest of the round.

Participants in the same segment act simultaneously. Each carries out their actions, then any damage, conditions, or spell effects from that segment are applied at the end of the segment, and characters only become Broken when that segment ends.

If a new combatant enters the fight, they roll initiative when they arrive, then roll normally in following rounds. If there are many NPCs with identical stats, the GM can roll once for the group and have them act together in that segment, while elite or named foes roll individually.

> **FAST SKILL ROLL INITIATIVE (OPTIONAL):** Use this for simple skirmishes, quick ambushes, or battles where you want the pace to stay fast and per-character order to matter less than overall momentum. Initiative is rerolled each round.
>
> At the start of each round, every player rolls **AGILITY**. If each player gets at least one ⚔️, they act before their enemies. If they have no ⚔️, they act after the enemies. The exact order among players does not matter. The enemies never need to roll.
>
> The side that initiated combat adds +2 dice to all initiative rolls in round 1 only. Players with talents that allow initiative rerolls can reroll once and keep the better result, while other players cannot reroll or push the roll. Enemies that can draw two or more initiative cards act before all players who rolled fewer than two ⚔️.
...
Another term used to keep track of time in the game is the turn. A turn is about 15 minutes long, and it is used primarily when exploring adventure sites.

**EXAMPLE**

_On their way from the old ruined castle, a few cheap gems and old books being all they have to show for it, Nirmena the half-elf and Tyrgar the dwarf are spotted by three orcs at the edge of the forest. They grin and get ready for a fight. At the start of the round, everyone rolls **AGILITY** and **WITS** together. Nirmena's unmodified **AGILITY** is 4 and she rolls 2 ⚔️, giving her segment 6. Tyrgar's unmodified **AGILITY** is 3 and he rolls 1 ⚔️, giving him segment 4. The orcs roll as individuals, landing on segments 2, 3, and 5. The order for this round is Nirmena (6), Orc 3 (5), Tyrgar (4), Orc 2 (3), Orc 1 (2). Next round, they roll again and the order may change._

### SLOW ACTIONS & FAST ACTIONS

When it is your turn to act, you can perform _one slow action and one fast action, or two fast actions_. A _slow action_ usually consists of rolling for a skill. A _fast action_ is quicker and doesn't always require a dice roll, but can do so. See the lists of typical slow and fast actions on the next page. How these work is explained in detail in the sections Close Combat and Ranged Combat.

**PREREQUISITES**: Some actions can only be performed under certain conditions. Often this is a feature of the weapon you are using.
...
| Treat Wounded | No enemy at Arm's Length, 1 hour  | Healing/Performance |
| Dragging      | One free hand, target is helpless | Move                |
...
| Brace          | Pointed Polearm or Long-Reach weapon                               | —                    |
| Intercept      | Spend both actions, enemy moving into reach                        | —                    |
| Lock Shields   | Shield, and at least one ally in the same zone doing the same      | —                    |
...
| Draw Grimoire  | You are a Druid or a Sorcerer                                      | —                    |
| Use Item       | Varies                                                             | Varies               |

### DESCRIBE YOUR ACTIONS

When it’s your turn, state which actions you wish to perform and roll dice as needed. Some actions will let your opponent perform a _reactive action_ – see Close Combat and Ranged Combat.

**SPELL ACTION ECONOMY:** Drawing a grimoire is a fast action if it is easily accessible. Using a grimoire is a free action and requires that you hold it with at least one hand. Casting a spell is a slow action, while power words are fast actions.

### HELPING OTHERS

If you _help_ another player character or NPC to perform an action, it costs you one action of the same type (slow or fast). You must that you help someone before any dice are rolled. Helping others breaks the initiative order in the round.
...
In typical conflicts, each NPC rolls initiative. If there are a lot of NPCs and the number of combatants is above ten, the GM will split the NPCs into groups. All the NPCs with identical stats will form a single group, and the GM will roll once per group instead of one per individual. All the NPCs in a group act at the same time in the turn order. In what order they act individually within the group is up to the GM.

#### MONSTERS

The kind of horrible creatures that are commonly called monsters don't act as regular enemies in combat. Monsters roll for initiative normally, but then follow their own rules.
...
The combat area is divided into _zones_. A zone is a room or an area of ground. How big a zone is varies - from a few steps across up to about 25 meters. A zone is generally smaller in difficult terrain than in open terrain. The distance from one zone to the next is called one segment.
...
The GM has drawn up a quick map of the area where Tyrgar and Nirmena face the orcs.

When combat starts, the adventurers are in a zone next to the orcs. The adventurers' zone is _open_ while the orcs' zone is _rough_.
...
**ROUGH:** You must roll _MOVE_ when you _RUN_ into the zone. Failure means you managed to get into the zone, but you fall down.
...
To move during combat, you use the _RUN_ action (fast) or _CRAWL_ (slow). This moves you from one zone to a neighboring zone, or between _NEAR_ and _ARM'S LENGTH_ distance from an enemy in the zone you are already in. No roll is required to _RUN_, unless you're moving into a _ROUGH_ zone, in which case you need to roll for _MOVE_ (see above).
...
When attacking a mounted opponent, you suffer a -1 penalty. Polearms ignore this penalty and can also add their item bonus when attempting to SHOVE a mounted opponent.

**DOORS:** You can open an unlocked door with the use _item_ action (fast). A locked door must either be opened by picking the lock or by breaking it down. A typical wooden door can take 5 points of damage before it falls apart. More sturdy doors require more force, and really massive doors also have an Armor Rating.

#### FLEEING THE CONFLICT

If you're losing a fight, it might be better to retreat and perhaps return with back-up. If you want to leave the conflict immediately, and you don't have any enemies at _arm's length_, you can roll for _move_ - a successful roll means you manage to get away somehow, and the conflict is over.

You cannot _flee_ in this way if you are trapped or surrounded. The GM has final say. You cannot use your roll to move past an opponent - you must _flee_ in the same direction you came from. The GM can modify your roll depending on the terrain and the distance to the nearest opponent, see the table below.

| FLEEING | RANGE      | MODIFICATION |
| ------- | ---------- | ------------ |
|         | Near       | -1           |
|         | Short      | 0            |
|         | Long       | +1           |
...
If the roll fails, you remain in combat with your opponent and cannot get away - you remain at the same distance as you were before. The GM can also inflict some additional negative consequence for you. You can attempt to _flee_ again in the next round.

**EXAMPLE**

The adventurers and the orcs are in neighboring zones. The distance between the groups is one segment, i.e., short range. Tyrgar tries to _flee_ across the broken bridge, but the orcs are already close behind him. He fails the roll and stays at the same distance, with their blades still near enough to strike.

#### MORALE & ROUT (OPTIONAL)

Not every fight ends with one side dead. When a side loses heart, the blades may still be in their hands, but the fight is already turning. This rule is for NPC groups of combatants only. Players do not roll morale for their own characters; they judge their own fear and keep their own skins on the line.

Make a morale check at the end of a round for the enemy side when one of these happens:

- its leader is Broken or killed
- half its fighters are Broken, dead, or have fled
- it faces a monstrous or demonic display that shakes the whole side
- it suffers a fear attack with 3 or more ⚔️, and there is a way to run
- it is plainly outmatched and has no real hope of winning

Each side has a morale rating in dice. Hardened enemies usually have 6 dice. Add dice for veterans, fanatics, or warriors with the upper hand. Take dice away for frightened rabble, hungry beasts, pressed men, or foes already half broken. When several such factors apply, each after the first changes the rating by reducing 1 die. When a morale check is called for, roll that many dice. If the roll succeeds, do not check again for two rounds. After that, repeat the check as normal. If the roll fails, the side must at once choose one of these:

- fall back and try to _flee_
- throw down weapons and surrender, but only if mercy seems possible
- cower, scatter, bargain, or break formation
- if there is no way out, keep fighting in fear

Those who fight on in fear because all other options are impossible, suffer a -2 penalty to all rolls for as long as the fear holds. This does not apply to those incapable of fear.

Undead, demons, monsters, and true fanatics are usually immune unless the GM decides otherwise.
...
**SNEAK ATTACK:** When you stalk someone and your attack catches them unawares, it's called a sneak attack. First, roll an opposed roll for _sneak_. You get a modification depending on how close you are - see the table below. If you want to attack in close combat, you usually have to get within _arm's length_. If you fail, your opponent spots you at your starting distance - roll for initiative.

If you succeed, you get a free action (slow or fast, but not both) before you roll for initiative. Your target cannot _dodge_ or _parry_ a sneak attack. Sneak attacks are always done individually, by one attacker against one target.
...
**AMBUSH:** A special kind of sneak attack is an ambush - you lie in wait for your enemy and attack when he passes. When you ambush someone, you roll _sneak_ as described above, but with a modification of +2, since it is the target and not the attacker that is moving.

Ambushes can be carried out by a group and against a group of targets. This follows the usual rules for _sneak_ - for the attackers (whoever has the lowest _sneak_ skill level roll) and for the targets (whoever has the highest _scout_ roll).

**STEALTH MODIFIERS:** You take a cumulative -1 penalty to _sneak_ if you have donned a HEAVY armor or are holding a large shield. You take a further -1 penalty for each negative MOVE modifier your gear applies to you. If both your hands are free or if you only hold TINY or LIGHT items, you instead gain a +1 _sneak_ modifier.

### CLOSE COMBAT

When you attack in close combat, you use the _melee_ skill. Close combat usually happens at _arm's length_. With some weapons, you can attack from _near_ distance (see the weapons, below).

**WEAPONS:** When you enter close combat, it is wise to have a weapon in hand. Weapons give you a Gear Bonus for your roll, increase the damage caused and allow you to perform actions that are harder or even impossible without a weapon. Several close combat weapons are described in the table below.

**SHIELD:** Carrying a shield is also very useful in close combat. You can _parry_ attacks with it, but you can also use it to _shove_ opponents to the ground. Shields are described in the table below.
...
**GRAPPLE:** You grab hold of your opponent. Roll for _melee_, as no weapon can be used. Can be _dodged_ or _parried_. If the attack succeeds, both you and your opponent fall to the ground. The opponent drops any weapon they were holding, and cannot move. The only action they can perform is _break free_ - which is a slow action and requires them winning an opposed _melee_ against you. While you are grappling, the only action you can perform is a _grapple attack_. It works like _punch/kick/bite_, but is a fast action and cannot be _dodged_ or _parried_. If you are grappling and only hold a LIGHT or TINY melee weapon, you do not need to drop it. You still cannot use it until you break free, but you may add its item die to any BREAK FREE or GRAPPLE attempts.
...
**HOLD FAST:** You settle into a guarded stance and give ground only inch by inch. Until the start of your next turn, choose either +1 to **PARRY** or +1 to **DODGE**. While you _hold fast_, enemies need one extra ⚔️ to **SHOVE**, **DISARM**, or **GRAPPLE** you. The effect ends early if you _run_, _shoot_, or make a slow attack action.

**SHOVE:** You try to push your enemy to the ground. Roll **MELEE**. A weapon with the **HOOK** feature or a shield can be used. If your opponent has higher Strength than you, two ⚔️ are required to succeed; otherwise, one is enough. If the attack is successful, your opponent falls to the ground and is prone (however, see the **STEADY FEET** talent). Any additional ⚔️ have no effect. Your opponent can **DODGE** to avoid your shove. **PARRYING** a shove requires a shield.

**DISARM:** You try to knock the weapon from your opponent's hands. Roll **MELEE** and your weapon's Gear Bonus. Disarming an opponent wielding a one-handed weapon requires one ⚔️, a two-handed weapon requires two (however, see the **FIRM GRIP** talent). If your opponent's **AGILITY** is not Broken, they may choose to oppose your **DISARM** attempt. If they do, they take all ⚔️ as **AGILITY** damage, or all ⚔️ minus one if they wield a two-handed weapon. If their **AGILITY** is not Broken after taking this damage, they are not disarmed. Otherwise, your opponent drops their weapon, which lands at _arm's length_. Any excess ⚔️ have no effect. You cannot **DISARM** a shield. Your enemy can **PARRY** or **DODGE** your disarm.
...
**INTERCEPT:** You prepare a reactive stance that anticipates an opponent's attack. During your turn, you spend both your FAST and SLOW actions. Until the start of your next turn, you may **ATTACK** an enemy that moves to engage you or **SHOOT** an enemy that moves into the range of your weapon. You can do this only once per round and act before the enemy. Your attack cannot be **PARRIED**, **DODGED**, or intercepted by another **INTERCEPT**, but you cannot **PARRY** or **DODGE** until the start of your next turn after you **INTERCEPT**.

**DRAGGING:** To drag a helpless person (such as a character with Broken Strength due to damage) during combat, a helping ally needs at least one hand free and must roll **MOVE** and get at least one ⚔️. This is a SLOW action. You may also move as part of this action, but you can only enter or leave a zone with difficult terrain if you make another **MOVE** roll. Any free attacks targeting the helpless person instead target the ally helping them.

**LOCK SHIELDS:** Requires a shield and at least one ally in the same zone also taking this action with a shield in hand. Until the start of your next turn, ranged attacks against all of you suffer a -1 penalty, and enemies need one extra ⚔️ to **SHOVE** any of you. If an enemy moves to _arm's length_ with one of you, that enemy is considered engaged with all of you who have locked shields. The effect ends for anyone who _runs_, _retreats_, falls prone, or uses **SHOVE**.
...
The fight between the adventurers and the orcs begins. Orc 1 uses his two actions to _run_ toward Tyrgar, first from _short_ to _near_ distance, then to _arm's length_. Orc 2 does the same, but approaches Nirmena. Then it's Tyrgar's turn. His axe is already drawn, so he uses his fast action to _shove_ Orc 1 to the ground, which succeeds.
...
| SHOOTING RANGE | RANGE MODIFICATION |
| -------------- | ------------------ |
| Arm's Length   | -3/+3              |
| Near           | —                  |
| Short          | -1                 |
...
**READY WEAPON**: Fast action. Before you can fire your bow or sling, you must **READY** it - prepare the weapon by knocking an arrow or placing a stone in your sling. Once you have **READYED** your weapon, you can't take any slow action other than **SHOOT** and no fast action other than **AIM** (see below) - if you do anything else, you must **READY** the weapon again before you can shoot. Note that the _fast shooter_ talent allows you to fire bows and slings without using an action to _ready_ the weapon.

Crossbows don't need to be _readied_. Instead, you must _load_ a crossbow (slow action) before each shot. You can carry a _loaded_ crossbow around as long as you like.

Bolts/quarrels are slightly cheaper than arrows: around 1 silver with iron heads and around 3 copper with wooden heads.

**AIM:** Fast action. Before you _shoot_, you can _aim_. This gives you +1 die against a moving target, or +2 dice if the target has not used a **MOVE** action during their last round. You must _aim_ and _shoot_ in the same round - you cannot save the bonus for a later round. Please note that you cannot _ready_ your weapon, _aim_ and _shoot_ in the same round, as that is a total of three actions. It's possible if you have the _fast shooter_ talent, however.

You can't _aim_ at an aware opponent at _arm's length_ - he is too close for you to be able to draw a bead on.

**SHOOT:** Slow action. Roll for _marksmanship_ and the weapon's Gear Bonus. Can be dodged, but to _parry_ a ranged attack the target must have a shield. If you hit, your target takes Weapon Damage 🩸 on his Strength. For every additional ⚔️ rolled, the 🩸 increases by one. If the target suffers a critical injury, roll on the table for stab wounds if you used a bow or throwing knife and use the table for blunt force if you used a sling or a thrown rock.

**DODGE:** Fast reactive action. You throw yourself out of the way of your opponent's attack. Roll _MOVE_ (not _MELEE_). Every ⚔️ you roll eliminates one ⚔️ from the attacker's roll. Any excess ⚔️ have no effect. When you _dodge_, you fall prone. You can choose to remain standing, at the cost of a -2 penalty.

**PARRY:** Fast reactive action. Ranged attacks count as _stab_ attacks when parrying. To _parry_ a ranged attack, you must have a shield or a weapon with the _parrying_ feature. Roll using _MELEE_ and the weapon's Gear Bonus. Every ⚔️ you roll eliminates one ⚔️ from the attacker's roll. Any excess ⚔️ have no effect.

#### REACTIVE ACTIONS

Just like in close combat, your opponent must declare if he intends to _dodge_ or _parry_ before you roll to attack. _Dodge_ and _parry_ are reactive actions and break the normal initiative order in the round.

**EXAMPLE**

The last remaining orc _retreats_ and then _runs_. In the next round, Tyrgar draws his crossbow and then _shoots_. The range is now short which gives Tyrgar a -1 penalty. He rolls two ⚔️, and with the crossbow's Weapon Damage of 2 he inflicts 3 points of 🩸.

### SOCIAL CONFLICT

Sometimes, you can make things go your way without resorting to violence. Instead, you trick or convince your opponent without drawing your weapon. This might even be possible in the midst of combat, if the GM judges it plausible. For non-violent conflicts, you use the _MANIPULATION_ skill.

What you ask of your opponent or what you want them to do must be within reason - no NPCs will agree to do anything or act completely against their own interests, no matter how good your roll is.

**MONSTERS:** Many monsters are so savage or dim-witted that they simply cannot be swayed by words, no matter how silvery your tongue is. Read more about monsters in the bestiary.

#### RESOLUTION

When you try to convince or bluff someone, make an opposed roll of _MANIPULATION_ versus your opponent's _INSIGHT_. It only counts as a (slow) action for you.

If you successfully _MANIPULATE_ your opponent, they must either do what you want or immediately attack you with physical violence. Even if your opponent chooses to do what you want, they can still demand something in return. The GM decides what that entails, but it should be reasonable enough for you to be able to meet those demands. It is up to you to accept the agreement or not.

#### NEGOTIATING POSITION

Your chances of _MANIPULATING_ someone successfully are affected by your negotiating position, which is determined by the GM. Each of following factors modifies your roll by +1:
...
When you want to _MANIPULATE_ a whole group, you usually address the group's leader or spokesperson. Remember that your roll is modified by -1 if your opponent has more people on their side. If you reach an agreement with the leader, the rest of the group usually follows. If there is no given leader it's harder - every single opponent acts individually.

#### RANGE

To _MANIPULATE_ someone, you usually need to be in _NEAR_ range - but if it is applicable to the situation, the actions can be used at _SHORT_ or even _LONG_ range. The GM modifies your roll negatively if she deems that the range impairs your negotiating position (see above).

#### ARTIFACTS

Some valuable or very impressive artifacts can give a Gear Bonus to your _MANIPULATION_ roll.

### WEAPONS

As an adventurer, you always want a weapon close at hand - you never know where danger lurks. The tables and images on the following pages show various kinds of common weapons in the Forbidden Lands. Unique and powerful artifacts are described elsewhere in this document.
...
| BONUS    | Indicates how many Gear Dice you can roll when using the weapon. Remember that the Gear Bonus can be reduced if you push your roll - the bonus decreases by one for every die you have rolled. If the Gear Bonus is reduced to zero, the weapon breaks and needs to be fixed using the _CRAFTING_ skill. |
| DAMAGE   | Indicates your base Weapon Damage, i.e., how many points of damage to Strength your opponent suffers if your attack is successful. If you roll extra ⚔️, you deal additional damage.                                                                                                                     |
| RANGE    | Indicates the maximum range at which the weapon can be used.                                                                                                                                                                                                                                             |
| COST     | Indicates the weapon's typical price in silver coins. The actual price can vary depending on supply and demand at the location.                                                                                                                                                                          |
| FEATURES | Indicates which features the weapon has. Possible features are described below and referenced in the Close Combat section.                                                                                                                                                                               |
...
**HIGH-VELOCITY:** Opponents can only PARRY in melee. Opponents suffer a -2 penalty to DODGE.
...
**MELEE/RANGED:** The weapon can be used both in melee and as a thrown weapon.

**MISFIRE:** If the attack suffers one or more item die 💀, the weapon inflicts its base damage on the wielder instead of the target.

**PARRYING:** The weapon can be used to PARRY.

**PIERCE ARMOR:** The target rolls their armor rating with a -3 penalty. The penalty cannot reduce the Armor Rating below 1.
...
**TRAPPING:** Opponents suffer a -2 penalty to PARRY DISARM attempts.

**UNARMED:** Counts as an unarmed attack for talents and other rules that reference unarmed attacks.

**WINDLASS:** The weapon cannot be loaded without a windlass. Setting the windlass is a fast action. The loading action is a slow action.

#### ARMOR FEATURES

**TOUGH:** The item soaks the first damage it receives. It cannot soak damage through the item die, until repaired, and the TOUGH modifier is repaired in the same way as a regular item die. If it already has a bonus die, you can apply a checkbox after the modifier, like “TOUGH [X]”.
...
- **SHORT-REACH:** Shorter than arm's length, like knives, shivs, and fists. You can only attack if you are already inside your opponent's reach.
...
**DAMAGE TO STRENGTH:** Bleeding wounds, broken bones, and pain. This is the default type of 🩸. If the type of 🩸 is not specified, it's always 🩸 to Strength.

**DAMAGE TO AGILITY:** Physical fatigue and exhaustion.

**DAMAGE TO WITS:** Fear, panic, and confusion. Read more under Fear Attacks.

**DAMAGE TO EMPATHY:** Despair, mistrust, sorrow.

#### SUFFERING DAMAGE

You can suffer 🩸 in many ways. These are the three most common:

**BY ROLLING** 💀 when you push a roll. When pushing, you suffer one point of 🩸 to the attribute used for every 💀 you roll.

**FROM ATTACKS** whenever someone successfully attacks you in close combat or with a ranged weapon, you suffer 🩸 to Strength equal to the Weapon Damage plus one for every additional ⚔️ your opponent rolls.

**FROM MAGIC.** Spells cast by Sorcerers and Druids can give you 🩸 of all types. Read more in Chapter 6.

### ARMOR

By wearing leather, chainmail, or a full suit of plate armor you can protect your body from 🩸 to Strength. See the list of various kinds of armor, below. Armor doesn't provide protection from other kinds of 🩸, or from 🩸 you inflict on yourself when you push a roll.

The effect of armor is determined by its Armor Rating. When you suffer 🩸 from a physical attack, you roll a number of Gear Dice equal to the Armor Rating. Every 💀 you roll decreases the 🩸 by one. This roll does not count as an action and cannot be pushed.

If any 🩸 penetrates your armor, its Armor Rating is decreased - every 💀 rolled reduces the Armor Rating by one. If the armor absorbed all the 🩸, any 💀 rolled has no effect. Armor can be repaired. Natural armor is not degraded in this way.
...
All helmets also have an extra effect: If you suffer one of the critical injuries, 6's for a slash wound, 64 for a stab wound, or 64-66 for blunt force, roll a number of Gear Dice equal to the Armor Rating of the helmet. If you roll one or more 💀, the critical injury is changed to the 11-12 blunt force injury instead. This will reduce the Armor Rating of the helmet to zero however.

#### ARMOR WEIGHT

Armor and all clothes increase their weight category by one if they are not worn, from TINY -> LIGHT -> NORMAL -> HEAVY -> 2x HEAVY. If they have any negative MOVE modifiers when worn, those modifiers are added to their weight when not worn. Mail (chainmail) and similar items are an exception and do not increase in weight while not worn.

#### ARMOR FIT

Use these optional rules for more historical realism.

**Found Armor Fit:** When armor is found rather than made for the wearer, roll a D6 to determine the original build and compare it to the wearer.

| D6  | Build   | Typical Kin                        |
| --- | ------- | ---------------------------------- |
| 1-2 | Slim    | Elves, Goblins, Halflings (common) |
| 3-4 | Average | Humans (default mix)               |
| 5-6 | Broad   | Orcs; Wolfkin (common)             |

**Kin Tendencies:** Elves, goblins, and halflings are predominantly slim. Orcs are predominantly broad. Wolfkin are commonly broad or slim. Dwarves have a distinct short, broad build; goblins and halflings share a similar small height range.

**Height Mismatch:** If the armor does not match the wearer's height, it cannot be donned completely.

**Body Type Mismatch Penalty:** If the found armor's body type does not match the wearer, the wearer suffers either -1 MOVE or -1 Armor Rating (choose when the armor is donned) until it is adjusted.

**Adjustment:** Resizing armor takes one Quarter Day by a Smith at a Smithy and requires a CRAFTING roll, as per Repair. On success, remove the mismatch penalty.
...
| ------------ | ----- | ------ | ----- | ---- | ---------------------------------- |
| Buckler      | +1    | 1      | Arm   | 4    | Light, Blunt, Parrying             |
| Small Shield | +1    | —      | —     | 6    | Light                              |
| Large Shield | +2    | —      | —     | 15   |                                    |
| Tower Shield | —     | —      | —     | 20   | Cover 6 (set), Heavy, Cannot parry |

The tower shield grants cover 6 only when set. Setting or picking it up is a fast action, and you cannot move while it is set. One additional person sharing the wielder's space can benefit from the cover.

### ARMOR & HELMETS

| GEAR                | ARMOR RATING | COST | BODY PART | FEATURES                                             |
| ------------------- | ------------ | ---- | --------- | ---------------------------------------------------- |
| Arming Doublet      | 2            | 2    | Body      | Light                                                |
| Leather             | 2            | 4    | Body      | Light                                                |
| Studded Leather     | 3            | 6    | Body      |                                                      |
| Gambeson            | 4            | 4    | Body      | Light                                                |
| Brigandine Armor    | 5            | 20   | Body      |                                                      |
...
| Mail Coif           | 2            | 8    | Head      | Light                                                |
| Closed Helmet       | 3            | 18   | Head      |                                                      |
| Great Helm          | 4            | 30   | Head      | Modifies Scout by -2                                 |

**EXAMPLE**

_Later on in the journey, Tyrgar gets in trouble again. The dwarf is hit by an angry minotaur's greataxe. The attack inflicts a whopping 4 points of damage. Tyrgar wears chainmail and an open helmet, for a total Armor Rating of 8. He rolls eight Gear Dice and gets one ⚔ and one 🖤. Tyrgar suffers 3 points of damage and his Armor Rating is decreased by 1. Tyrgar chooses to let his chainmail take the hit and its Armor Rating is thus decreased to 5._

### BROKEN

When an attribute score reaches zero, you are _Broken_. This means that you are put out of action in one way or another. Exactly what it means to be Broken depends on what attribute has been depleted:

#### STRENGTH

You're knocked senseless. Roll for a critical injury for the type of damage you suffered. If you're not dead, you can only _crawl_ and mumble through the pain. You can't perform any other actions and you can't roll for any skills.

**CRITICAL INJURY MODIFIERS:** When you take a critical injury, roll D66 with a modifier. Treat any modified result below 11 as 11 (no critical injury). The modifier is based on how many points of damage were dealt after armor. Some effects that modify damage, such as Axe Mastery, count toward this modifier.

| MODIFIER (RANGE) | CRIT BY DAMAGE DEALT |
| ---------------- | -------------------- |
| -10 (11-56)      | 1                    |
| +/-0 (11-66)     | 3                    |
| +10 (21-66)      | 5                    |
...
If your Strength reaches zero because you pushed a roll instead of suffering direct damage, you remain standing but are completely spent. You cannot attempt further Strength-based rolls. You may spend 1 WP to attempt a single Strength roll, using two Base Dice as if your Strength were 2. If you push that emergency roll, you collapse and become helpless even if it shows ⚔️ unless you pay 1 WP for each 💀 rolled. Any damage that hits you while you are in this state immediately knocks you prone and defenseless.

#### AGILITY

You collapse from exhaustion. You can only _crawl_ and wheeze. You can't perform any other actions and you can't roll for any skills.

If your Agility hits zero because you pushed a roll, you keep your feet but strain muscles and lose fine control. You cannot make further Agility-based rolls unless you spend 1 WP to attempt a single Agility roll using two Base Dice (treat your Agility as 2). If you push that attempt, you fall helpless unless you spend 1 WP for each ⚔️ rolled. All of your Fast actions become Slow actions, and you only get one action per round until you recover.
...
The fastest way to recover from being Broken is for someone else to treat you with **HEALING** (for Strength or Agility) or **MOTIVATION** (for Wits or Empathy). These are not combat actions and take one hour to perform.
...
You can receive **HEALING** whenever you have attribute damage, not only when Broken. The healer chooses whether to restore Strength or Agility.

A successful **HEALING** roll takes one hour, stops the timer of the critical injury being treated during the treatment, and applies the recovery at the end of this period. A character can benefit from one **HEALING** roll per day and cannot be **HEALED** again until the same Quarter Day on the next day has passed. A failed **HEALING** roll still consumes the daily limit for that character. A failed **HEALING** roll takes a Turn (15 minutes), during which the healer identifies that they cannot help the wounded ally during the current Quarter Day.

- A character trained in **HEALING** can treat multiple wounded or Broken allies during the same Quarter Day, up to 1 + their skill rank, but each patient requires a separate roll.
- Patients who **HIKE** or perform other strenuous activity during _JOURNEY_, or who are treated outside a _CAMP_, are healed with a negative modification based on the conditions, to a minimum of -1.
- If the healer treats themselves, the roll is modified by -1 (or more, depending on circumstances such as weather and hygiene).
- Help from others can be received only if the assistant is trained in **HEALING** and is not a patient.
- The healer cannot _REST_ while treating the wounded during that Quarter Day.

#### MOTIVATION

The rules for **HEALING** apply similarly when a counselor uses **MOTIVATION** with a **PERFORMANCE** roll to lift exhausted minds. The performer can recover Wits or Empathy, just as **HEALING** recovers Strength or Agility, and the performer can _REST_ after tending to the group.

**MOTIVATION** is usually performed during the _REST_ Quarter Day, for one hour before sleep. The performer must describe the title or premise of a story they tell the troubled characters, but does not need to perform it in full unless the table agrees.
